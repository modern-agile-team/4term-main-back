...
import { ApiIsUserHasUnreadNotices } from './swagger-decorator/is-user-has-unread-notices.decorator';
...
export class NoticesController {
  ...
  @ApiIsUserHasUnreadNotices()
  @Get('/unread')
  async isUserHasUnreadNotices(
    @GetUser() userNo: number,
  ): Promise<APIResponse> {
    const isUserHasUnreadNotices: boolean =
      await this.noticesService.isUserHasUnreadNotices(userNo);

    return {
      response: { isUserHasUnreadNotices },
      msg: '유저 읽지 않은 알림 조회',
    };
  }
}
